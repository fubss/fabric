...
func TestMain(m *testing.M) {
	flogging.ActivateSpec("boltdbhelper=debug")
	os.Exit(m.Run())
}
...
func TestIterator(t *testing.T) {
	...
	t.Run("test-first-prev", func(t *testing.T) {
		...
		require.True(t, itr.First())
		// Next itr.Prev() was commented because
		// this case seems never happening in HLF.
		// Cursor in boltdb will be nil forever after
		// itr.Prev() for the first key in the cursor.
		// Therefore itr.Next() in checkItrResults() will point to nil.
...
func TestDrop(t *testing.T) {
	...
	// negative test
	p.Close()
	require.EqualError(t, db2.deleteAll(), "internal boltdb error while obtaining db iterator: database not open")
}
